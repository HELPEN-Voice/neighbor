...
async def test_marion():
    ...
    target = await finder.get_target_parcel(
        search_mode="COORDS", lat=40.5267, lon=-97.5956
    )
    ...
    if target:
        ...
        adjacent = await finder.get_adjacent_parcels(
            target_geometry=target["geometry"], target_pin=target["pin"]
        )
        ...
        for n in neighbors[:5]:
            print(
                f"  - {n['name']} ({n['entity_type']}): {len(n['pins'])} parcels, Adjacent: {n['owns_adjacent_parcel']}"
            )

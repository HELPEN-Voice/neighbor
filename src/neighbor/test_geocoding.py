    ...
    for test in test_locations:
        ...
        try:
            ...
            if result['state'] != test['expected_state']:
                print(f"  ❌ State mismatch: expected {test['expected_state']}, got {result['state']}")
                passed = False
            else:
                print(f"  ✓ State matches: {result['state']}")
            
            if result['county'] and test['expected_county_contains'] not in result['county']:
                print(f"  ❌ County mismatch: expected to contain '{test['expected_county_contains']}', got {result['county']}")
                passed = False
            elif result['county']:

...
async def test_pipeline():
    ...
    for idx, neighbor in enumerate(json_data.get("neighbors", [])):
        try:
            ...
            print(
                f"   ✓ Neighbor {idx + 1} ({neighbor['name']}) validated successfully"
            )
        except Exception as e:
            validation_errors.append(f"Neighbor {idx + 1}: {e}")
            print(
                f"   ✗ Neighbor {idx + 1} ({neighbor.get('name', 'Unknown')}) validation failed:"
            )
            print(f"      Error: {e}")
    ...
    print(
        f"     - Mary Johnson stance: '{mary_after.get('stance')}' (should be 'unknown')"
    )
    print(
        f"     - Mary Johnson influence_level: '{mary_after.get('influence_level')}' (should be 'unknown')"
    )
    print(f"     - Mary Johnson flags: {mary_after.get('flags')} (should be [])")

    # Verify the downgrade worked
    assert mary_after.get("stance") == "unknown", (
        "Stance should be downgraded to 'unknown'"
    )
    assert mary_after.get("influence_level") == "unknown", (
        "Influence level should be downgraded"
    )
    ...
    if deep_dive_html.exists():
        ...
        if (
            json_data.get("overview_summary")
            and json_data["overview_summary"][:30] in content
        ):
